...
/*   Updated: 2024/07/20 14:41:56 by athonda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_dup(long *nbrs, int count, int argc, char **str)
{
	...
	while (i < count - 1)
	{
		...
		while (j < count)
		{
			if (nbrs[i] == nbrs[j] || nbrs[i] > 2147483647 || nbrs[i] < -2147483648)
			{
				write (2, "Error\n", 6);
				free_array_l(nbrs);
				if (argc == 2)
...
int	check_int(long *nbrs, int count, int argc, char **str)
{
	int	i;
	...
	while (i < count)
	{
		if (nbrs[i] < -2147483648 || nbrs[i] > 2147483647)
		{
			write (2, "Error\n", 6);
			free_array_l(nbrs);
			if (argc == 2)
...
int	check_arg(char **str, int argc)
{
	...
	if (check_dup(tmp, count, argc, str))
		return (1);
	if (check_int(tmp, count, argc, str))
		return (1);
